...
def render_dynamic_section_html() -> str:
    ...
    if not PUBLIC_MODE:
        return (
            ...
            '<h2 class="mode-title">'
            '<span class="mode-num">2</span>动态执行（Docker 跑代码）'
            '</h2>'
            '<p class="mode-desc">在 VM 的 Docker 容器里 <code>python script.py</code> / '
            '<code>bash script.sh</code> 跑 skill，<code>strace</code> 抓系统调用、<code>tcpdump</code> 抓网络包。'
            '<strong>不调 Claude API，零成本</strong>。</p>'
            '<div class="upload-row">'
            '<form method="post" action="/asg/vm_paper_run" enctype="multipart/form-data" class="upload-card">'
            '<span class="upload-card-tag">A · 现场跑（连 VM 真执行）</span>'
            '<label>Skill zip / 单文件<input class="file-input" type="file" name="archive" required></label>'
            '<button class="btn-submit" type="submit">▶ 上传 + VM 容器执行</button>'
            '</form>'
            '<form method="post" action="/asg/vm_ingest" class="upload-card">'
            '<span class="upload-card-tag">B · 吃旧证据（离线打分）</span>'
            '<label>本地 Skill 目录路径<input class="file-input" name="skill_path" placeholder="asg/samples/data_thief" required></label>'
            '<label>证据目录<input class="file-input" name="evidence_dir" placeholder="analysis_results/asg/data_thief/vm_ssh_logs" required></label>'
            '<button class="btn-submit" type="submit">▶ 离线吃证据</button>'
            '</form>'
            '</div>'
            '</section>'
            '<section class="mode-section mode-3">'
            '<h2 class="mode-title">'
            '<span class="mode-num">3</span>Docker 里跑 Claude（验证 Agent 抗诱导）'
            '</h2>'
            '<p class="mode-desc">在 VM 的 Docker 容器里启动 <strong>Claude CLI</strong>，把 skill 装到 '
            '<code>~/.claude/skills/</code>，给 Claude 一个 prompt 让它读这个 skill 并使用，<code>strace</code> 录下真实行为。约 ¥1.2/次。</p>'
            '<form method="post" action="/asg/vm_ssh_run" enctype="multipart/form-data" class="upload-card" style="max-width:640px;">'
            '<span class="upload-card-tag">让 Claude 在容器里使用此 Skill</span>'
            '<label>Skill zip / 单文件<input class="file-input" type="file" name="archive" required></label>'
            '<button class="btn-submit" type="submit">▶ 上传 + 启动 Claude 容器</button>'
            '</form>'
            '</section>'
        )
    # 公开模式：纯说明
    return (
        ...
        '<h4>这一档做什么</h4>'
        '<p>把上传的 skill 解压到 VM 里的 Docker 容器，直接 <code>python script.py</code> / <code>bash script.sh</code> 真跑，'
        '<code>strace</code> 抓系统调用、<code>tcpdump</code> 抓网络包。证据强度最高，但会消耗服务器资源也可能落盘真实痕迹。</p>'
        '<h4>本地启用方法</h4>'
        '<pre># Windows PowerShell\nRemove-Item Env:\\ASG_PUBLIC_MODE -ErrorAction SilentlyContinue\npython web_ui\\app.py</pre>'
        '<p>启用前需要：① 填好 <code>asg/vm_config.json</code>；② VM 上装 Docker + strace + tcpdump。详细安全边界见 <code>asg/README.md</code>。</p>'
        '</div>'
        '</section>'
        '<section class="mode-section mode-disabled">'
        '<h2 class="mode-title">'
        ...
        '<div class="docs-panel">'
        '<h4>这一档做什么</h4>'
        '<p>在 VM 容器里启动 <strong>Claude CLI</strong>，把 skill 装到 <code>~/.claude/skills/</code>，让 Claude 真的去读并使用这个 skill，<code>strace</code> 录下真实行为。回答核心问题：<strong>真 Claude 会不会被 SKILL.md 骗着干坏事</strong>。约 ¥1.2/次（kuaipao.ai）。</p>'
        '<h4>本地启用方法</h4>'
        '<pre># 1. 填好 asg/vm_config.json 的 remote_anthropic_api_key\n# 2. VM 装 Claude CLI + Docker\n# 3. 关公开模式重启:\nRemove-Item Env:\\ASG_PUBLIC_MODE -ErrorAction SilentlyContinue\npython web_ui\\app.py</pre>'
        '</div>'
        '</section>'
    )
