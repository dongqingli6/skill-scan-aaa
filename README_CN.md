...
适合直接拿到项目文件夹的用户。两个启动器作用相同，按系统选一个：

**Windows**

1. 进入项目根目录，双击 `打开网页.bat`。
2. 首次启动会自动安装两个依赖（`anthropic`、`paramiko`），耗时数十秒。
3. 启动成功后，浏览器会自动打开 `http://127.0.0.1:8765/`。

环境要求：Windows 10 / 11，已安装 Python 3.10 或以上（安装时勾选 "Add Python to PATH"）。

**macOS**

1. 首次使用前，打开终端 Terminal，进入项目根目录后执行一次：
   ```bash
   chmod +x 打开网页.command
   ```
   说明：从 zip 解压的文件在 macOS 上默认没有可执行权限，需要赋权一次。已通过 git 克隆的项目通常不需要此步骤。
2. 在 Finder 中双击 `打开网页.command`，首次启动会弹出终端窗口并自动安装依赖。
3. 启动成功后，浏览器会自动打开 `http://127.0.0.1:8765/`。

环境要求：macOS 11 或以上，已安装 Python 3.10 或以上（可从 python.org 下载，或使用 Homebrew 安装：`brew install python`）。

**关闭服务**：关闭命令行 / 终端窗口，或在窗口内按 Ctrl + C。
